...
// CODE HERE
const variable_function = (parameter1, parameter2) => console.log(`The first parameter is ${parameter1}. The second parameter is ${parameter2}`)

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
variable_function("food", "color")
...
greeting("Mel", "Jes", fullName)


//greeting("John", "Doe", fullName => console.log(`Hello, my full name is ${fullName}`));
...
function pricesPlusTax(prices, callback) {
    ...
    for(let i = 0; i < prices.length; i++) {
        totalCost.push(prices[i] + (prices[i] * 0.20))
    }
    callback(totalCost)
}
