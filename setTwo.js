...
//forEach method (returns the element, returns the element INDEX)
function each(name,callback) {
  name.forEach((element,pizza) => {
    callback(element, pizza)
  });
}
...
function getUserById(object_arr, id, callback) {
  for(let i in object_arr) {
    if(object_arr[i].id === id ) {
      callback(object_arr[i])
    }
  }
}

// one line with ternary
const getUserById = (object_arr, id, callback) => object_arr.forEach(element => element.id === id ? callback(element) : null)
